...
async function start() {
  const asyncFunc1 = await myAsync1().then((result) => console.log(result))
  const asyncFunc2 = await myAsync2().then((result) => console.log(result))
  const asyncFunc3 = await myAsync3().then((result) => console.log(result))
  const asyncFunc4 = await myAsync4().catch((error) => console.log(error.message))
}
